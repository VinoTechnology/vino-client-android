...
class jFrustrum : public jSingleton<jFrustrum>
{
public:
	void get(JNIEnv* env, jobject& mf, float& left, float& right, float& up, float& bottom, float& near, float& far);
private:
	jFrustrum(JNIEnv* env);
	...
	jfieldID mnear;
	jfieldID mfar;
	friend class  jSingleton<jFrustrum>;
};
